...
const Navbar = () => {
  ...
  console.log("hellow", user?.email);
  const [orders, setOrders] = useState([]);

  // taking all selected product price and quantity
  const allPrices = orders?.map((order) => parseInt(order.productPrice));
  const allQuantity = orders?.map((order) => parseInt(order.porductQuantity));


  // calculating the price of selected product
  let sum = 0;
  for (let i = 0; i < allPrices.length; i++) {
    sum += allPrices[i] * allQuantity[i];
  }
  // const subtotal = allPrices?.concat(allQuantity)?.reduce((t,n)=>t+n)
  console.log("ok", sum);


  // Taking the orders
  console.log("nice", orders);
  useEffect(() => {
